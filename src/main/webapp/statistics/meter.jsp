...
/*           for(var i = 1; i< list.length; i ++){
          ...
          plot1 = $.jqplot('chart_panel_3', [line],{
              animate: true,
              animateReplot: true,
  ...
  function lineup() {
	    ...
	    //alert("테스트");
	    var line = co.get('pebbles');
	    if (co.options.show==false){ co.options.show = true;}
	    else {co.options.show = false;}
	    co.draw(plot1);
	}
	  ...
	  <c:when test="${sessionScope.consumer_id != null && sessionScope.admin=='Y'}">
		  ...
		  <table>
		    <tr>
		      ...
		      <td>
		      <button type="button" onclick="lineup();">평균추가</button>
		      </td>
		    </tr>
		  </table>
	  </c:when>
